...
subplot(212); plot(S_bat_case1(:,1)); hold on; plot(P3); hold on; plot(S_bat_case1(:,2))
...
for time = 1:N
    ...
    P_bat_da = S_bat_case1(time,1);
    Q_bat_da = S_bat_case1(time,2);
...
figure; subplot(211); plot(t2,Envelope(:,1)); hold on; plot(t2,Envelope(:,2)); hold on; plot(t,S_bat_case1(:,1))
...
profit_only_arbitrage_dayAhead =  0.25*(sum(Total_cost_DA'*subplus(S_bat_case1(:,1))-kappa*Total_cost_DA'*subplus(-S_bat_case1(:,1))))
...
C3_pf_load_agg = pf_load_agg_case3;
C3_Voltage = Voltage_rt;
C3_Out_b_mat = Out_b_mat;
C3_P_load_total = P_load_total;
C3_Q_load_total = Q_load_total;
C3_battery_ch_level = Out_b_mat(:,3);
